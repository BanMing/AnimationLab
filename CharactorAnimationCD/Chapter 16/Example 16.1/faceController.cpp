...
void FaceController::Update(float deltaTime)
{
	...
	if(speech1 < 0.0f){m_speechIndices[0] = rand() % (int)m_pFace->m_speechMeshes.size(); speech1 = 0.0f;}
	float speech2 = sin(time);
	if(speech2 < 0.0f){m_speechIndices[1] = rand() % (int)m_pFace->m_speechMeshes.size(); speech2 = 0.0f;}
