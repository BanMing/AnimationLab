...
public:
	UploadBuffer(ID3D12Device* device, UINT elementCount, bool isConstantBuffer) :mIsConstantBuffer(isConstantBuffer)
	{
		...
		if (isConstantBuffer)
		{
			mElementByteSize = CalcConstantBufferBytesSize(sizeof(T));
		}

		device->CreateCommittedResource(&D3D12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD),
