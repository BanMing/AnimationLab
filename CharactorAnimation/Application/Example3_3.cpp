...
void Example3_3::OnRender()
{
	...
	if (SUCCEEDED(m_pDevice->BeginScene()))
	{
		//Render Soldier
		{
			m_drone.Render(NULL, m_pSkinningsEffect);
		}
...
{
	if (m_pSkinningsEffect != NULL)
	{
		m_pSkinningsEffect->Release();
		m_pSkinningsEffect = NULL;
	}
}
...
{
	m_pSkinningsEffect->OnLostDevice();
}

void Example3_3::OnDeviceGained()
{
	m_pSkinningsEffect->OnResetDevice();
}
