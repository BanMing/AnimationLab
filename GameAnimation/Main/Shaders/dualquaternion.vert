...
// skinning uniforms, 
// index 0 is the real part of dual quaternion
// index 1 is the dual part of dual quaternion
...
vec4 transformPoint(mat2x4 dq, vec3 v)
{
    ...
    vec3 t = mulQ(conjugate, dual * 2.0f).xyz;
...
void main()
{
    ...
    if (dot(pose[joints.x][0], pose[joints.y][0]) < 0.0f){ w.y *= -1.0f; }
    if (dot(pose[joints.z][0], pose[joints.z][0]) < 0.0f){ w.z *= -1.0f; }
    if (dot(pose[joints.w][0], pose[joints.w][0]) < 0.0f){ w.w *= -1.0f; }
