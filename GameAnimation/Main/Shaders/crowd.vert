...
uniform vec3 model_rot[MAX_INSTANCES];
uniform vec3 model_sc[MAX_INSTANCES];
...
mat4 GetPose(int joint, int instance)
{
	...
	vec4 pos1 = texelFetch(animTex, ivec2(x_now, (x_next + 0)), 0);
	vec4 rot1 = texelFetch(animTex, ivec2(x_now, (x_next + 1)), 0);
	vec4 scl1 = texelFetch(animTex, ivec2(x_now, (x_next + 2)), 0);
