...
int Track<T, N>::FrameIndex(float time, bool looping)
{
	...
	else
	{
		...
		// The Sample function always needs a current and next frame
		// the next frame is found by adding to the result of the FrameIndex function
		// When the time equals the time of the last frame, the two frames that need 
		// to be interpolated are still the secode-to-last frame and the last frame.
...
// When given a time, this function needs to adjust the time to 
// be in therange of the start/end frames of the track
template<typename T, int N>
...
T Track<T, N>::SampleLinear(float time, bool looping)
{
	...
	// we need interpolate,so not the last frame
