...
// each elment = dot(row,col)-> MAD
mat4 operator*(const mat4& a, const mat4& b)
{
	return mat4(
		M4D(0, 0), M4D(1, 0), M4D(2, 0), M4D(3, 0), // col0
		M4D(0, 1), M4D(1, 1), M4D(2, 1), M4D(3, 1), // col1
		M4D(0, 2), M4D(1, 2), M4D(2, 2), M4D(3, 2), // col2
		M4D(0, 3), M4D(1, 3), M4D(2, 3), M4D(3, 3));// col3
}
...
// each elment = dot(row,col)-> MAD
vec4 operator*(const mat4& m, const vec4& b)
{
	return vec4(
		M4V4D(0, b.x, b.y, b.z, b.w),
		M4V4D(1, b.x, b.y, b.z, b.w),
		M4V4D(2, b.x, b.y, b.z, b.w),
		M4V4D(3, b.x, b.y, b.z, b.w));
}
